        ...
        try:
            ...
            if USE_SUGGEST_RESPONSES:
                ...
                try:
                    ...
                    pass
                if images_embed:
                    await message.followup.send(response, view=MyView(chatbot, conversation_style, suggest_responses), embeds=images_embed, wait=True)                
                elif link_embed:
                    await message.followup.send(response, view=MyView(chatbot, conversation_style, suggest_responses), embed=link_embed, wait=True)
                else:
                    await message.followup.send(response, view=MyView(chatbot, conversation_style, suggest_responses), wait=True)
            else:
                if images_embed:
